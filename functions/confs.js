...
exports.remove = (guild) => {
  if(guildConfs.has(guild.id)) {
    guildConfs.remove(guild.id)
  }
  fs.removeSync(path.resolve(dataDir + path.sep + guild.id + ".json"), function (err) {
...
exports.addKey = (key, defaultValue, type=null) => {
  type = type ? type : defaultValue.constructor.name;
